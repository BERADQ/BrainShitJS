module.exports = (code, cPath, name) => {
    const fs = require('fs');
    ...
    console.log(code);
    let theStream = code.match(/.{2}?/g);
    console.log(theStream);
    let sCode = getSCode(theStream, codFormat);
    let ws = fs.createWriteStream(pathUtil.join(cPath, `${name}.bns`));
    ws.write(sCode);
    ws.end();
    console.log(sCode);
    return true;
    function getSCode(str2, codFormat) {
        const getto = (char) => {
            let temp = 0xD;
            switch (char) {
                case '1':
